...
import com.saiyi.gymequipment.equipment.model.request.DeleteFitnessService;
...
public class FitnessCenterModel extends ModelImpl {
    ...
    public void deleteFitness(int idFitnessCenter, BaseResponseListener<String> listener) {
        DeleteFitnessService deleteFitnessService = createRetorfitService(DeleteFitnessService.class);
        JSONObject result = new JSONObject();
        try {
            result.put("idFitnessCenter", idFitnessCenter);
        } catch (JSONException e) {
            e.printStackTrace();
        }
        RequestBody body = RequestBody.create(MediaType.parse("application/json"), result.toString());
        deleteFitnessService.deleteFitnessCenters(UserHelper.instance().getToken(), body)
                .unsubscribeOn(Schedulers.io())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
