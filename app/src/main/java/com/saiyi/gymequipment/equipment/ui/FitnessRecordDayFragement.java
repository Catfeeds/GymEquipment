...
import com.saiyi.gymequipment.equipment.presenter.FitnessRecordDayPresenter;
...
public class FitnessRecordDayFragement extends BKMVPFragment<FitnessRecordDayPresenter>{
    ...
    @Override
    public FitnessRecordDayPresenter initPresenter(Context context) {
        return new FitnessRecordDayPresenter(context);
    }
    ...
    protected void initData() {
        super.initData();
        getPresenter().getFitnessRecordsDay();
        getPresenter().getTimesTotalRecordDay();
    }
    ...
    private void initColumnChart() {
        ...
        for (int i = 0; i < mFitnessRecords.size(); i++) {
            values = new ArrayList<SubcolumnValue>();
            values.add(new SubcolumnValue(mFitnessRecords.get(i).getFrduration().intValue(), Color.parseColor("#00E2A5")));
            Column column = new Column(values);
            columns.add(column);
            mAxisXValues.add(new AxisValue(i).setLabel(StringUtils.fromDateToHHmm(mFitnessRecords.get(i).getFrcreatetime())));
        }
        //数据小于一个页面指定数，补空
        if (mFitnessRecords.size() < COLUMNS_NUMBER) {
