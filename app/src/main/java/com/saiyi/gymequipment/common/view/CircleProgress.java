...
public class CircleProgress extends View {
    ...
    //    private int[] mGradientColors = {Color.GREEN, Color.YELLOW, Color.RED};//渐变颜色
    private int[] mGradientColors = {Color.parseColor("#1BEAEA"), Color.parseColor("#1BEAEA")};//默认为同一种颜色
    //当前进度，[0.0f,1.0f]
    ...
    private void init(Context context, AttributeSet attrs) {
        mContext = context;
        mDefaultSize = CircleProgressUtils.dipToPx(mContext, CircleProgressUtils.DEFAULT_SIZE);
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        antiAlias = typedArray.getBoolean(R.styleable.CircleProgressBar_antiAlias, CircleProgressUtils.ANTI_ALIAS);
        ...
        mHintSize = typedArray.getDimension(R.styleable.CircleProgressBar_hintSize, CircleProgressUtils.DEFAULT_HINT_SIZE);

        mValue = typedArray.getFloat(R.styleable.CircleProgressBar_value, CircleProgressUtils.DEFAULT_VALUE);
        mMaxValue = typedArray.getFloat(R.styleable.CircleProgressBar_maxValue, CircleProgressUtils.DEFAULT_MAX_VALUE);
        //内容数值精度格式
        mPrecision = typedArray.getInt(R.styleable.CircleProgressBar_precision, 0);
        mPrecisionFormat = CircleProgressUtils.getPrecisionFormat(mPrecision);
        mValueColor = typedArray.getColor(R.styleable.CircleProgressBar_valueColor, Color.BLACK);
        mValueSize = typedArray.getDimension(R.styleable.CircleProgressBar_valueSize, CircleProgressUtils.DEFAULT_VALUE_SIZE);
        ...
        mUnitSize = typedArray.getDimension(R.styleable.CircleProgressBar_unitSize, CircleProgressUtils.DEFAULT_UNIT_SIZE);

        mArcWidth = typedArray.getDimension(R.styleable.CircleProgressBar_arcWidth, CircleProgressUtils.DEFAULT_ARC_WIDTH);
        mStartAngle = typedArray.getFloat(R.styleable.CircleProgressBar_startAngle, CircleProgressUtils.DEFAULT_START_ANGLE);
        mSweepAngle = typedArray.getFloat(R.styleable.CircleProgressBar_sweepAngle, CircleProgressUtils.DEFAULT_SWEEP_ANGLE);

        mBgArcColor = typedArray.getColor(R.styleable.CircleProgressBar_bgArcColor, Color.WHITE);
        mBgArcWidth = typedArray.getDimension(R.styleable.CircleProgressBar_bgArcWidth, CircleProgressUtils.DEFAULT_ARC_WIDTH);
        ...
        mAnimTime = typedArray.getInt(R.styleable.CircleProgressBar_animTime, CircleProgressUtils.DEFAULT_ANIM_TIME);

        //改动代码，实现自适应颜色
        mHintColor = typedArray.getColor(R.styleable.CircleProgressBar_arcColors, Color.BLACK);
        mGradientColors = new int[2];
        mGradientColors[0] = mHintColor;
        mGradientColors[1] = mHintColor;
...
//        mValuePaint.setTypeface(Typeface.DEFAULT_BOLD);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(CircleProgressUtils.measure(widthMeasureSpec, mDefaultSize),
                CircleProgressUtils.measure(heightMeasureSpec, mDefaultSize));
    }
    ...
    private float getBaselineOffsetFromY(Paint paint) {
        return CircleProgressUtils.measureTextHeight(paint) / 2;
    }
    ...
    public void setPrecision(int precision) {
        ...
        mPrecisionFormat = CircleProgressUtils.getPrecisionFormat(precision);
    }
