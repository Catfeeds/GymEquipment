...
public class TagAliasOperatorHelper {
    private static final String TAG = JPushLib.TAG + "_Tag_Alias";
                ...
                case DELAY_SEND_ACTION:
                    if (msg.obj != null && msg.obj instanceof TagAliasBean) {
                        ...
                        if (context != null) {
                            ...
                            JLogger.e(TAG, "#unexcepted - context was null");
                        }
                    } else {
                        JLogger.w(TAG, "#unexcepted - msg obj was incorrect");
                    }
                    break;
                case DELAY_SET_MOBILE_NUMBER_ACTION:
                    if (msg.obj != null && msg.obj instanceof String) {
                        JLogger.i(TAG, "retry set mobile number");
                        sequence++;
    ...
    public void handleAction(Context context, int sequence, String mobileNumber) {
        ...
        JLogger.d(TAG, "sequence:" + sequence + ",mobileNumber:" + mobileNumber);
    ...
    public void handleAction(Context context, int sequence, TagAliasBean tagAliasBean) {
        ...
        if (tagAliasBean == null) {
            JLogger.w(TAG, "tagAliasBean was null");
            return;
        }
        ...
        if (tagAliasBean.isAliasAction) {
            switch (tagAliasBean.action) {
                ...
                case ACTION_SET:
                    ...
                    break;
                default:
                    JLogger.w(TAG, "unsupport alias action type");
                    return;
            }
        } else {
            switch (tagAliasBean.action) {
                ...
                case ACTION_CLEAN:
                    ...
                    break;
                default:
                    JLogger.w(TAG, "unsupport tag action type");
                    return;
            }
        }
    }
    ...
    private boolean RetryActionIfNeeded(int errorCode, TagAliasBean tagAliasBean) {
        if (!JPushUtil.isConnected(context)) {
            JLogger.w(TAG, "no network");
            return false;
        }
        //返回的错误码为6002 超时,6014 服务器繁忙,都建议延迟重试
        if (errorCode == 6002 || errorCode == 6014) {
            JLogger.d(TAG, "need retry");
            if (tagAliasBean != null) {
    ...
    public void onTagOperatorResult(Context context, JPushMessage jPushMessage) {
        ...
        JLogger.i(TAG, "action - onTagOperatorResult, sequence:" + sequence + ",tags:" + jPushMessage.getTags());
        JLogger.i(TAG, "tags size:" + jPushMessage.getTags().size());
        init(context);
        //根据sequence从之前操作缓存中获取缓存记录
        TagAliasBean tagAliasBean = (TagAliasBean) setActionCache.get(sequence);
        if (tagAliasBean == null) {
            JLogger.d(TAG, "获取缓存记录失败");
            return;
        }
        if (jPushMessage.getErrorCode() == 0) {
            JLogger.i(TAG, "action - modify tag Success,sequence:" + sequence);
    ...
    public void onCheckTagOperatorResult(Context context, JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
        JLogger.i(TAG, "action - onCheckTagOperatorResult, sequence:" + sequence + ",checktag:" + jPushMessage.getCheckTag());
        init(context);
        //根据sequence从之前操作缓存中获取缓存记录
        TagAliasBean tagAliasBean = (TagAliasBean) setActionCache.get(sequence);
        if (tagAliasBean == null) {
        ...
        if (jPushMessage.getErrorCode() == 0) {
            JLogger.i(TAG, "tagBean:" + tagAliasBean);
            setActionCache.remove(sequence);
    ...
    public void onAliasOperatorResult(Context context, JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
        JLogger.i(TAG, "action - onAliasOperatorResult, sequence:" + sequence + ",alias:" + jPushMessage.getAlias());
        init(context);
        //根据sequence从之前操作缓存中获取缓存记录
        TagAliasBean tagAliasBean = (TagAliasBean) setActionCache.get(sequence);
        if (tagAliasBean == null) {
        ...
        if (jPushMessage.getErrorCode() == 0) {
            JLogger.i(TAG, "action - modify alias Success,sequence:" + sequence);
            setActionCache.remove(sequence);
            String logs = getActionStr(tagAliasBean.action) + " alias success";
            JLogger.i(TAG, logs);
            JPushMsgCenter.instance().dispatchOnSetAliasSuccess();
        } else {
            String logs = "Failed to " + getActionStr(tagAliasBean.action) + " alias, errorCode:" + jPushMessage.getErrorCode();
            JLogger.e(TAG, logs);
            JPushMsgCenter.instance().dispatchOnSetAliasFaild();
            if (!RetryActionIfNeeded(jPushMessage.getErrorCode(), tagAliasBean)) {
                JLogger.d(TAG, logs);
            }
        }
    ...
    public void onMobileNumberOperatorResult(Context context, JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
        JLogger.i(TAG, "action - onMobileNumberOperatorResult, sequence:" + sequence + ",mobileNumber:" + jPushMessage.getMobileNumber());
        init(context);
        if (jPushMessage.getErrorCode() == 0) {
            JLogger.i(TAG, "action - set mobile number Success,sequence:" + sequence);
            setActionCache.remove(sequence);
        } else {
