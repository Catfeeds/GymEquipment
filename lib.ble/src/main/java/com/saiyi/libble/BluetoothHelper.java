...
public class BluetoothHelper {
    ...
    private BleConnectStateListener mBleConnectCaseListener;//蓝牙连接状态回调

    private SearchResultListener mSearchResultListener;//扫描结果回调

    private SystemBleOpenListener mSystemBleOpenListener;//系统蓝牙打开 关闭监听
...
//                if (device.getName().contains("JSQ")) {
                ...
                Log.e(TAG, "名字：" + device.getName() + "--MAC:" + device.getAddress());
                if (mSearchResultListener != null) {
                    mSearchResultListener.searchResult(addressBean);
                }
...
//                }
            ...
            public void onSearchStopped() {
                Log.e(TAG, "停止扫描");
                if (mSearchResultListener != null) {
                    mSearchResultListener.searchStopped();
                }
            ...
            public void onSearchCanceled() {
                Log.e(TAG, "扫描取消");
                if (mSearchResultListener != null) {
                    mSearchResultListener.searchCanceled();
                }
    ...
    public interface SearchResultListener {
        void searchResult(BleAddressBean bean);
     ...
     */
    public void connectBle(final String mac) {
        Log.e(TAG, "蓝牙连接ing");
        if (mClient != null) {
            mClient.connect(mac, getBleConnectOptions(), new BleConnectResponse() {
                @Override
                public void onResponse(int code, BleGattProfile data) {
                    Log.e(TAG, "蓝牙连接结果：" + code);
                    if (mConnectBleListener != null) {
                        mConnectBleListener.connectResult(code, data, mac);
                    }
                }
            });
        }
    ...
    public interface ConnectBleListener {
        void connectResult(int code, BleGattProfile data, String mac);
    }
     ...
     * 设置蓝牙打开状态监听
     */
    public void setBleOpenListener() {
        if (mClient != null) {
    ...
    public interface SystemBleOpenListener {
        void bleOpenResult(boolean openOrClosed);
    }

    public void setSystemBleOpenListener(SystemBleOpenListener mSystemBleOpenListener) {
        this.mSystemBleOpenListener = mSystemBleOpenListener;
    }
     ...
     */
    public void setBleConnectStatusListener(String mac) {
        if (mClient != null) {
     ...
     */
    public void sendNotify(String mac, String serviceUUID, String passagewayUUID) {
        if (mClient == null) {
            return;
        }
        mClient.notify(mac, UUID.fromString(serviceUUID), UUID.fromString(passagewayUUID), new BleNotifyResponse() {
            ...
            public void onNotify(UUID service, UUID character, byte[] value) {
                Log.e(TAG, "onNotify value:" + byteArrayToHexString(value));
     ...
     * @param data           需要写入的数据
     */
    public void sendMessage(String mac, String serviceUUID, String passagewayUUID, byte[] data) {
        if (mClient == null) {
            return;
        }
    ...
    private BluetoothStateListener mBluetoothStateListener = new BluetoothStateListener() {
        ...
        public void onBluetoothStateChanged(boolean openOrClosed) {
            Log.e(TAG, "蓝牙状态：" + openOrClosed);
            if (mSystemBleOpenListener != null) {
                mSystemBleOpenListener.bleOpenResult(openOrClosed);
            }
        }
    };
    ...
    private BluetoothBondListener mBluetoothBondListener = new BluetoothBondListener() {
        ...
        public void onBondStateChanged(String mac, int bondState) {
            Log.e(TAG, "蓝牙地址：" + mac + "     连接状态：" + bondState);
        }
    };
    ...
    private BleConnectStatusListener mBleConnectStatusListener = new BleConnectStatusListener() {
        ...
        public void onConnectStatusChanged(String mac, int status) {
            if (mBleConnectCaseListener != null) {
    ...
    public void setBleConnectStateListener(BleConnectStateListener mBleConnectCaseListener) {
